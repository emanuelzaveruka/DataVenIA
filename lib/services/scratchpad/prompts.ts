...
Gere: resumo do caso (caseSummary), fatos relevantes (facts), questões jurídicas identificadas (legalIssues), holdings (uma entrada por proposição jurídica distinta, cada uma com proposition, stance e reasoning), pontos favoráveis e contrários (favorablePoints/contraryPoints), fatos que distinguem este caso de outros (distinguishingFacts), leis e precedentes citados (citedLaws/citedPrecedents), trechos candidatos a evidência (evidenceCandidates, cada um com quote, context e purpose — o identificador é atribuído pelo sistema, não peça nem invente um), uma nota de relevância (relevance.score entre 0 e 1 e relevance.reason), um nível de confiança (confidence entre 0 e 1) e o status da análise ("VALID", "PARTIAL" ou "FAILED").`;
}
