...
describe("buildRelatorioDashboard", () => {
  ...
  it("separa decisões CITADAS de decisões ANALISADAS", async () => {
    ...
    //
    // Não se exige que sejam > 0: desde que a fixture passou a usar host `.invalid` (HU-27), os
    // achados de demonstração são recusados como fonte oficial e viram omissão. Zero citação é
    // resultado legítimo, e o painel tem estado próprio para ele.
    expect(painel.amostra.decisoesCitadas).toBeLessThanOrEqual(painel.amostra.decisoesAnalisadas);
    expect(painel.amostra.camaras).toBeLessThanOrEqual(painel.amostra.decisoesCitadas);
    expect(painel.amostra.relatores).toBeLessThanOrEqual(painel.amostra.decisoesCitadas);
  });
