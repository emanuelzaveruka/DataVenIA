...
import { SearchPlanSchema } from "../../schemas/search-plan.schema";
...
/**
 * Checkpoint humano entre `QUERY_GENERATION` e `SEARCH`.
 *
 * O que está sendo protegido aqui não é a UI: é que a pausa aconteça ANTES de qualquer busca (o
 * ponto do fluxo em que corrigir um termo ainda é barato) e que a retomada use os termos do
 * usuário, não os do modelo — se ela reexecutasse a geração de queries, a edição seria decorativa.
 */
describe("runPipeline — plano de busca revisado pelo usuário", () => {
  /** Espiona quais queries chegaram ao provider, preservando o comportamento da fixture. */
  ...
  it("para depois de gerar as queries e devolve a proposta, sem tocar na busca", async () => {
    ...
    for await (const event of runPipeline(
      {
        ...
        pauseAfterQueries: true,
      },
      { repository, llmProvider: stubLlmProvider(), jurisprudenceProvider: provider },
    )) {
      events.push(event);
    }
    ...
    expect(plano && plano.type === "plan" && plano.proposal.queries).toHaveLength(
      QUERY_PLAN.queries.length,
    );
    expect(plano && plano.type === "plan" && plano.proposal.legalIssues[0]?.topic).toBe(
      CASE_ANALYSIS.legalIssues[0]?.topic,
    );

    // O ponto da pausa: nenhuma busca foi disparada, e não há relatório.
    expect(recebidas).toHaveLength(0);
    expect(events.some((event) => event.type === "result")).toBe(false);
    expect(
      stageEvents(events).some((event) => event.event.stage === "SEARCH"),
    ).toBe(false);
  });

  it("retoma da busca com os termos do usuário e aplica os filtros escolhidos", async () => {
    const repository = createInMemoryRepository();
    const primeira = spyProvider();

    for await (const _ of runPipeline(
      {
        runId: "run-plan-0002",
        traceId: "trace-plan-0002",
        file: {
          name: "peticao.txt",
          size: Buffer.byteLength(PETITION),
          type: "text/plain",
      ...
      { repository, llmProvider: stubLlmProvider(), jurisprudenceProvider: primeira.provider },
    )) {
      void _;
    }

    const segunda = spyProvider();
    const events: PipelineEvent[] = [];
    ...
    for await (const event of runPipeline(
      {
        runId: "run-plan-0002",
        traceId: "trace-plan-0002",
        // O usuário descartou a query principal do modelo, manteve a contrária e escreveu a sua.
        resume: {
          queries: [
            {
              query: "negativa de cobertura de procedimento prescrito pelo médico assistente",
              reason: "Termo acrescentado pelo usuário na revisão do plano de busca.",
              intent: "MAIN_THESIS",
              legalIssueId: "LI-01",
            },
            {
              query: QUERY_PLAN.queries[1]!.query,
              reason: "Busca jurisprudência contrária à tese.",
              intent: "CONTRARY",
              legalIssueId: "LI-01",
            },
      ...
      { repository, llmProvider: stubLlmProvider(), jurisprudenceProvider: segunda.provider },
    )) {
      events.push(event);
    }

    // Os termos buscados são os do usuário, não os que o modelo propôs.
    expect(segunda.recebidas.map((item) => item.query)).toEqual([
      "negativa de cobertura de procedimento prescrito pelo médico assistente",
      QUERY_PLAN.queries[1]!.query,
    ]);
    // A query principal que o modelo havia proposto foi descartada pelo usuário e NÃO foi buscada.
    expect(segunda.recebidas.map((item) => item.query)).not.toContain(QUERY_PLAN.queries[0]!.query);
    // E o recorte acompanha TODA query — aplicar em só algumas daria um total que não corresponde
    // a recorte nenhum no funil de HU-13.
    for (const recebida of segunda.recebidas) {
    ...
    const resultado = events.find((event) => event.type === "result");
    expect(resultado).toBeDefined();
    // A retomada não reexecutou a ingestão: o nome do arquivo veio do que foi persistido.
    expect(resultado && resultado.type === "result" && resultado.payload.fileName).toBe(
      "peticao.txt",
    );
  });

  it("recusa retomar uma execução que não tem estado persistido", async () => {
    const events: PipelineEvent[] = [];

    for await (const event of runPipeline(
      {
        runId: "run-que-nunca-existiu",
        traceId: "trace-plan-0003",
        resume: {
          queries: [
            {
              query: "qualquer",
    ...
    expect(erro && erro.type === "error" && erro.error.code).toBe("RUN_NOT_RESUMABLE");
  });

  it("recusa um plano sem nenhuma pesquisa por jurisprudência contrária (HU-11)", () => {
    const semContraria = SearchPlanSchema.safeParse({
      queries: [
        {
          query: "só a tese do cliente",
          reason: "r",
          intent: "MAIN_THESIS",
          legalIssueId: "LI-01",
        },
    ...
    expect(semContraria.success).toBe(false);
    expect(semContraria.success === false && semContraria.error.issues[0]?.message).toContain(
      "contrária",
    );
  });

  it("o recorte realmente restringe: câmara inexistente na base não devolve acórdão", async () => {
    const repository = createInMemoryRepository();

    for await (const _ of runPipeline(
      {
        runId: "run-plan-0004",
        traceId: "trace-plan-0004",
        file: {
          name: "peticao.txt",
          size: Buffer.byteLength(PETITION),
          type: "text/plain",
    ...
    for await (const event of runPipeline(
      {
        ...
        traceId: "trace-plan-0004",
        resume: {
          queries: QUERY_PLAN.queries as never,
          // A fixture só tem 3ª e 9ª Câmara Cível; o recorte abaixo não pode achar nada.
          filters: { judgingBody: "Turmas Recursais" },
        },
      },
      {
        repository,
    ...
    const erro = events.find((event) => event.type === "error");
    expect(erro && erro.type === "error" && erro.error.code).toBe(
      "NO_RELEVANT_JURISPRUDENCE_FOUND",
    );
  });
});